...
**Descrição:** Classe principal que contém o método `main()` e implementa a interface de usuário através de um menu interativo no console. Integra todas as funcionalidades do sistema.

### Atributos

| Atributo | Tipo | Modificador | Descrição |
|----------|------|-------------|-----------|
| `gerenciador` | `GerenciadorDespesas` | `static` | Instância única do gerenciador de despesas compartilhada por todos os métodos |
| `scanner` | `Scanner` | `static` | Objeto Scanner para leitura de entrada do usuário |
...
```
Ponto de entrada do programa. Exibe o menu principal e processa as escolhas do usuário em um loop até que a opção "Sair" seja selecionada.
...
```
Exibe o menu principal com todas as opções disponíveis no sistema.

**`entrarDespesa()`**
```java
private static void entrarDespesa()
```
Solicita ao usuário as informações necessárias (descrição, valor, categoria) e cadastra uma nova despesa através do gerenciador.

**`anotarPagamento()`**
```java
private static void anotarPagamento()
```
Lista as despesas pendentes e permite ao usuário registrar o pagamento de uma delas informando o ID.

**`listarDespesasPendentes()`**
```java
private static void listarDespesasPendentes()
```
Exibe as despesas em aberto e oferece um submenu para excluir despesas ou voltar ao menu principal.

**`listarDespesasPagas()`**
```java
private static void listarDespesasPagas()
```
Exibe as despesas pagas e oferece um submenu para excluir despesas ou voltar ao menu principal.

**`gerenciarTiposDespesa()`**
```java
private static void gerenciarTiposDespesa()
```
Funcionalidade parcialmente implementada. Atualmente permite gerar relatórios por categoria. O gerenciamento completo de tipos será implementado em versões futuras.

**`gerenciarUsuarios()`**
```java
private static void gerenciarUsuarios()
```
Funcionalidade ainda não implementada. Exibe mensagem informativa sobre implementação futura.

### Conceitos de POO Aplicados

- **Métodos estáticos:** Todos os métodos são estáticos pois não há necessidade de múltiplas instâncias da classe Main
- **Separação de responsabilidades:** Cada método trata de uma funcionalidade específica do menu
- **Reutilização:** O gerenciador é instanciado uma única vez e reutilizado em todas as operações
